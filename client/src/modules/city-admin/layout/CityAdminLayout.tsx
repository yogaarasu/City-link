...
const CityAdminLayout = () => {
  ...
  const navItems: NavItem[] = useMemo(
    () => [
      { to: "/city-admin/dashboard", label: t("dashboard"), icon: LayoutDashboard },
      { to: "/city-admin/manage-issues", label: t("manageIssues"), icon: Wrench },
    ],
    [t]
  );
  ...
  const sidebar = (
    <div className="flex h-full flex-col overflow-y-auto overflow-x-hidden">
      ...
      <div className="mt-auto px-3 pb-4">
        ...
        <Button
          variant="ghost"
          className="mb-1 w-full justify-start gap-3 py-3 text-lg hover:bg-muted hover:text-foreground"
          onClick={() => {
            toggleLanguage();
        ...
        <Button
          variant="ghost"
          className="mb-2 w-full justify-start gap-3 py-3 text-lg hover:bg-muted hover:text-foreground"
          onClick={() => {
            navigate("/city-admin/settings");
            setIsMobileMenuOpen(false);
          }}
        >
          ...
          <DropdownMenuTrigger asChild>
            <Button variant="outline" className="mb-2 h-auto w-full justify-start gap-3 py-3">
              ...
              <div className="flex min-w-0 flex-col items-start text-left">
                <span className="truncate text-sm font-semibold">{user?.name ?? "City Admin"}</span>
                <span className="truncate text-xs text-muted-foreground">{user?.role ?? t("cityAdminRole")}</span>
              </div>
            </Button>
          </DropdownMenuTrigger>
  ...
  return (
    <div className="h-svh overflow-hidden bg-muted/30">
      <header className="sticky top-0 z-40 flex items-center justify-between border-b border-border/70 bg-gradient-to-b from-background via-background/95 to-background/80 px-4 py-3 shadow-sm backdrop-blur-md transition-colors duration-300 md:hidden">
        <div className="flex items-center gap-3">
          ...
          <h1 className="flex items-center gap-1.5 text-lg font-semibold text-foreground">
            <img src="/citylink-logo-new.png" alt="CityLink logo" className="h-5 w-5 rounded-full bg-white/90 p-0.5 box-border object-contain dark:bg-black/40" />
            CityLink
          </h1>
        </div>
          ...
          >
            <div className="relative">
              ...
              <div className="flex flex-col items-center gap-2 pt-1 pb-0.5">
                ...
                <p className="max-w-full truncate text-sm font-semibold">{user?.name ?? "City Admin"}</p>
      ...
      <div className="flex h-[calc(100svh-57px)] md:h-svh">
        ...
        <main className="scrollbar-hide flex-1 overflow-y-auto p-4 md:p-8">
          <Outlet />
        </main>
      </div>
